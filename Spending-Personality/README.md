...
- 최근 5개 결과를 다시 여는 얕은 히스토리 흐름
- Vercel 기준 로컬 개발/배포 구조
- 기본 `verify` 스크립트
- 후속 task 를 위한 저장/공유 액션 확장 자리잡기
...
- 성공 결과는 브라우저 로컬에 최대 5개까지 저장되고, 최근 히스토리에서 다시 열 수 있습니다
- 샘플 소비 내역과 메모는 실제 캐릭터 생성 엔진에 연결되어 결과 카드와 최근 히스토리를 렌더링합니다
- 결과 계약은 `success / needs-more-data / parse-failed` 상태를 검증합니다
- 다음 task 에서 저장/공유 액션과 더 긴 기간의 기록 경험을 붙일 수 있습니다
