...
작성 2026-09-23 · 상태 **구현 (2026-09-23, 실기기 확인 대기)** · 코드 `Color-Moments/`
...
- **처음 켤 때 양쪽에 기록이 있으면 합친다.** 같은 `cloudID` 면 하나만 남긴다. 남길 기록은 **`id` 문자열이 작은 쪽**(구현 검토에서 바꿈 — `capturedAt` 은 저장 정밀도가 기기마다 달라 두 기기가 서로 상대를 이긴다고 판정할 수 있었다). 내용은 `MomentMerge.combine` 으로 양쪽을 합친다(단어는 남는 쪽에 있으면 그것, 없으면 다른 쪽).
- 받은 `Moment` 에는 `fileName` 이 없으므로 **`DayStore.applyRemote` 가** 자리 이름을 강제한다: cloudID 가 있으면 `asset-<fnv1a(cloudID)>`, 없으면 `"remote-" + id`. assetID·originalName 은 비운다. 이런 자리 이름은 입양(`fileBacked`) 대상이 아니다.
- `days.json` 날짜는 소수 초까지 저장한다(옛 초 단위 파일도 읽는다). 동기화 비교는 1ms 미만 차이를 같게 본다.
- 한 기기의 디버그 「전부 지우기」(`removeAll`)는 이 기기만 지운다 — 다른 기기로 번지지 않는다.
...
2. 기존 안전장치 그대로: 전체 접근(`.authorized`)일 때만 · 조회가 통째로 비면 안 지움 · 한 번에 `max(3, 20%)` 넘으면 그 회차 건너뜀 · 옵저버는 `removedObjects` 만. **옵저버 경로에도 같은 상한**(2026-09-24 최종 검토 — 「iCloud 사진 끄기 → iPhone 에서 제거」 한 번에 전부 `removedObjects` 로 올 수 있고, 그 삭제가 모든 기기로 번진다).
   - 폴링 정리는 못 찾은 assetID 를 지우기 전에 cloudID 로 한 번 더 찾는다 — 찾으면 새 assetID 로 갈아 끼우고 지우지 않는다(복원 뒤 로컬 ID 가 바뀐 경우).
3. 이 기기의 정리가 기록을 지우면 그 `Moment` 기록의 **CloudKit 삭제를 올린다.**
4. **다른 기기에서 온 삭제는 따른다** — 그 기기가 1·2 를 통과했다는 뜻이다. 이 기기가 고친 기록을 올리다 「서버에 없음(unknownItem)」이 오면 다시 만들지 않고 로컬에서도 지운다.
7. 앱을 켤 때 동기화를 **가장 먼저** 시작한다. 그 전에 생긴 변경은 저장소가 모아 두었다가 넘긴다.
...
- **증정은 어느 기기에서든 한 번만** (Tabber 결정). `GiftLog` 을 「마지막 하루(`lastGiftedDayKey`)」에서 **받은 하루 집합**으로 바꾸고 `Day.gifted` 로 동기화한다.
  ...
  - 판정은 **받은 날 중 가장 늦은 날 이하 전부 받은 것**(floor) — 여러 날 비운 뒤 가장 최근 하루만 주고 건너뛴 날이 나중에 튀어나오지 않게(2026-09-23 검토). 후보는 「가장 최근 자연히 끝난 하루」 먼저, 그 다음 「마무리한 오늘」. 다른 기기에서 받은 하루는 조용히 열린 하루로 보인다.
...
ColorMoments/Sync/
  ...
  CloudSync.swift                CKSyncEngine 위임자: 보낼 변경 모으기, 받은 변경 적용,
                                  엔진 상태 직렬화(Documents/sync-state.json), 계정 변화 처리
...
- 로그아웃·다른 계정으로 전환: 이 기기 기록은 **지우지 않는다**. 동기화 상태만 버리고 지금 계정에 전부 다시 올린다 — 다른 계정으로 바꾸면 두 계정 기록이 섞일 수 있고, 로그아웃 동안 다른 기기가 지운 기록이 되살아날 수 있다. 둘 다 드물어 「기록을 잃지 않는다」를 택했다(2026-09-24).
- 사용자가 iCloud 설정에서 몽돌 데이터를 지우면 로컬은 두고 옛 기록을 다시 올리지 않는다. 암호 재설정(`encryptedDataReset`)이면 전부 다시 올린다.
