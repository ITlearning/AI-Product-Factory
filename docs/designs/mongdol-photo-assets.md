...
- 권한은 **처음 찍을 때** 묻는다(첫 실행에 묻지 않는다). 거부하면 → 찍기는 되지만 저장할 곳이 없으므로 「사진 접근을 켜야 담을 수 있어요」 한 줄 + 설정 열기. 촬영물은 버린다(몽돌 폴더에 몰래 남기지 않는다).
...
- 정리 시점: 앱이 앞으로 올 때 + `PHPhotoLibraryChangeObserver` 가 삭제를 알릴 때. 한 번에 `PHAsset.fetchAssets(withLocalIdentifiers:)` 로 묶어 확인.
- 순수 판정 함수 `AssetReconciler.missing(ids:, found:, status:) -> [String]` 로 빼서 테스트: 제한 접근이면 늘 빈 배열.
- 지운 뒤 그 하루에 사진이 없으면 홈에서 조약돌이 사라진다. 알림·안내 없음(재촉 금지와 같은 결).

## 4. 코드 구조

```
Shared/Day/Moment.swift        fileName 은 옛 기록용으로 남기되 선택값. 새 사진은 assetID 만.
Shared/Day/ShotImage.swift     그리는 입구 하나 — assetID 가 있으면 AssetImageSource, 없으면 지금처럼 파일
ColorMoments/Library/
  AssetImageSource.swift       PHCachingImageManager 로 썸네일·원본 (앱 타깃, Photos)
  AssetSaver.swift             Data → PHAssetCreationRequest → localIdentifier
  AssetReconciler.swift        지운 사진 찾기 (순수 판정 + 실제 조회)
```

- `Shared/` 는 Photos 를 모른다 — `ShotImage` 는 앱 타깃이 꽂아 주는 `ImageSource` 프로토콜을 통해서만 에셋을 그린다. 확장은 파일 경로만 쓴다.
