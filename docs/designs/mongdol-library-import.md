...
| 증정은 자정에 하루 한 번 | 조약돌이 없던 지난 날에 올리면 **조용히** 홈에 조약돌이 생긴다. 증정 화면 없음. 그 하루에 봉인된 사진이 하나도 없으면 증정하지 않는다(`DayStore.hasSealedMoments`) |
...
```
앱 안 촬영 화면
  셔터 왼쪽 [사진첩] 버튼 (46pt, SF Symbol `photo.on.rectangle` 18pt 아이콘 — 썸네일 아님)
...
- 다 담기까지 선택 화면에 머물고, 끝나면 닫힌다. 담는 동안은 「담기 N」 버튼 자리에 진행 표시(`ProgressView`)가 대신 뜨고, 닫기도 비활성.
...
- 조약돌에 들어가는 사진 = 그 하루의 **봉인 시점** 전에 있던 사진.
  ...
  - 봉인 시점 = **그 하루가 끝나는 때(다음 날 04:00)**. `GiftLog` 는 날마다의 증정 시각을 남기지 않아 쓰지 않는다 — 결과는 같다.
  - 담은 사진은 `addedAt ≤ 봉인 시점` 이면 들어간다.
- **조약돌이 없던 지난 날**: 그 하루에 **가장 먼저 담긴 묶음**(`batchID`)이 조약돌을 만든다. 그 뒤 묶음부터 빠진다.
- 새로 담긴 사진은 **하루 상세 시간축(띠·눈금·사진)과 홈 사진 더미**에 보인다. 조약돌·이름·한 줄 말·홈 배경 번짐·하루 상세 헤더와 번짐·증정은 봉인된 사진만.
